...
jqueryWidget: {
    _init: function() {
        ...
        if (typeof(this.options.s) == "string")
            ...
            assert_is_arraylike(this.options.s, "Bad value for 's' option of OnlineJudgment.");
            this.words = this.options.s;
        }
        this.mode = dget(this.options, "mode", "self-paced reading");
        assert(this.mode == "self-paced reading" || this.mode == "speeded acceptability" || this.mode == "lexical decision",
               "Value of 'mode' option for OnlineJudgment controller must be either " +
        ...
        this.fixationCross = dget(this.options, "fixationCross", "+");
        this.SOA = dget(this.options, "SOA", 1000);
        ...
        assert(this.display == "dashed" || this.display == "in place",
               "Value of 'display' option for OnlineJudgment controller must be either " +
        ...
        assert(this.sentenceDescType == "md5" || this.sentenceDescType == "literal", "Bad value for 'sentenceDescType' option of OnlineJudgment.");
        ...
        this.resultsLines = [];
        if (this.mode == "self-paced reading" || this.mode == "lexical decision") {
        ...
        if (this.mode == "speeded acceptability") {
         ...
         else if (this.mode == "lexical decision") {
            var t = this;
            this.wordSpan.text(this.fixationCross)
            function LexDec () {
                t.showWord(0);
                t.previousTime = new Date().getTime();
                t.safeBind($(document), 'keydown', function(e) {
                    var time = new Date().getTime();
                    var code = e.keyCode;
                    var word = t.currentWord;
                    var rs = t.sprResults[0];
                    rs[0] = time;
                    rs[1] = t.previousTime;
                    rs[2] = String.fromCharCode(code);
                    if (code == 83 || code == 75) {
                        t.processSprResults();
            ...
            this.safeBind($(document), 'keydown', function(e) {
                var time = new Date().getTime();
                var code = e.keyCode;
// This is the part I'm changing. 32 is the code of the space key, so I just need to make two possible codes and add the code to the results file.
                if (code == 83) {
                    // *** goToNext() ***
    ...
    processSprResults: function () {
        for (var i = 0; i < this.sprResults.length; ++i) {
            this.resultsLines.push([
                ...
                ["KeyCode", this.sprResults[i][2]], // This is where it puts the keycode into the results file (hopefully)
